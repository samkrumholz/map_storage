...
for (yr in c("1940","1970","1990","2000","2010","2020")) {
  ...
  cat("\n=== Year", yr, "===\n")
  cat("Rows:", nrow(g_df), "\n")
  cat("school_age_share: median=", round(median(g_df$school_age_share, na.rm=TRUE)*100,1),
      "% NAs=", sum(is.na(g_df$school_age_share)), "\n")
  cat("private_school_share: median=", round(median(g_df$private_school_share, na.rm=TRUE)*100,1),
