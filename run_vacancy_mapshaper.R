# Compute vacancy stats from CSVs in R (no geometry), then let mapshaper join
library(tidyverse)

BASE <- "C:/Users/krumh/Claude/white_map"
OUT  <- file.path(BASE, "data")
TMP  <- file.path(BASE, "tmp_vac")
...
for (yr in names(cfgs)) {
  ...
  for (cf in sort(Sys.glob(cfg$csv_glob))) {
    ...
    v <- parse_codebook(cb)
    oc <- find_vars(v, cfg$occupied_pat); vc <- find_vars(v, cfg$vacant_pat)
  ...
  message("  occ=", paste(occ_cols, collapse=","), " vac=", paste(vac_cols, collapse=","))
  ...
  dat <- read_csv(csv_file, show_col_types = FALSE) %>%
    mutate(occ_pop  = rowSums(select(., all_of(occ_cols)), na.rm = TRUE),
           ...
           vac_share = round(if_else(total_hu > 0, vac_pop / total_hu, NA_real_), 6)) %>%
    select(GISJOIN, total_hu, vac_share)

  tmp_csv <- file.path(TMP, paste0("vac_", yr, ".csv"))
  write_csv(dat, tmp_csv)
  message("  Wrote stats CSV: ", nrow(dat), " rows")
}
message("\nCSV step done. Run mapshaper join next.")
